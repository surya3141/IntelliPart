...
class HackathonDemo:
    ...
    def __init__(self):
        """Initializes the Hackathon Demo, setting up paths and analytics engine."""
        # Construct an absolute path to the data file relative to this script's location
        base_dir = os.path.dirname(os.path.abspath(__file__))
        data_file_path = os.path.join(base_dir, "data", "training Dataset.jsonl")
        
        try:
            self.analytics = AdvancedAnalytics(data_file_path)
    ...
    def demo_business_impact(self):
        ...
        # --- ROI Calculation ---
        development_cost = 50000  # Estimated cost
        if annual_savings > 0:
            roi_percentage = (annual_savings / development_cost) * 100
            payback_months = (development_cost / annual_savings) * 12
            print(f"   📈 ROI: {roi_percentage:.1f}% (Payback in {payback_months:.1f} months)")
        else:
            print("   📈 ROI: Not applicable (no savings identified)")
